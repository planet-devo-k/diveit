...
- 대표적인 유틸리티 타입
  - Partial<T>: T 타입의 모든 속성을 선택적으로 만듦
  - Readonly<T>: T 타입의 모든 속성을 읽기 전용으로 만듦
  - Record<K, T>: K 타입의 키와 T 타입의 값을 가지는 객체 타입을 만듦
  ...
  - NonNullable<T>: T 타입에서 null과 undefined를 제외한 새로운 타입을 만듦
  - ReturnType<T>: 함수 타입 T의 반환 타입을 추출함
  - Parameters<T>: 함수 타입 T의 매개변수 타입들을 튜플 형태로 추출함
...
person1.name = ""; // ❌ name은 Readonly 프로퍼티입니다.
```

![utility type](./assets/utility-type.png)
...
- 특정 객체 타입의 모든 프로퍼티를 옵셔널 프로퍼티로 변환합니다.
  <br/>따라서 기존 객체 타입에 정의된 프로퍼티들 중 일부분만 사용할 수 있도록 도와주는 타입입니다.
...
- 그럼 이때 keyof T는 ‘title’ | ‘content’ | ‘tags’ | ‘thumbnailURL’이므로
  <br/>`Pick<T, Exclude<keyof T, K>>` 은 `Pick<Post, Exclude<'title' | 'content' | 'tags' | 'thumbnailURL' , 'title>` 이 됩니다.
  <br/>Exclude 타입은 2개의 타입 변수를 할당받는데 T로부터 K를 제거합니다.
  <br/>`Pick<Post, 'content' | 'tags' | 'thumbnailURL'>`
...
1. `<T extends (...args: any) => any>` 이 부분은 제약 조건입니다. T는 반드시 함수 형태여야 한다는 뜻입니다. 함수가 아닌 타입을 넣으면 에러가 발생합니다.
