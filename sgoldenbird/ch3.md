...
- 최상위에서만 훅을 호출해야 한다. 반복문이나 조건문, 중첩된 함수 내에서 훅을 실행할 수 없다. 이 규칙을 따라야만 컴포넌트가 렌더링 될때마다 항상 동일한 순서로 훅이 호출되는 것을 보장할 수 있다.
...
- 훅을 호출할 수 있는 것은 "리액트 함수 컴포넌트" 혹은 "사용자 정의 훅(커스텀 훅)" 두가지 경우 뿐이다. 일반 자바스크립트 함수에서는 훅을 사용할 수 없다.
  즉, 훅은 함수 컴포넌트 내부 또는 커스텀 훅 내부에서만 사용할 수 있다.
...
- 링크드 리스트(Linked List)
  ...
  - 링크드 리스트는 메모리상에서 연속된 공간에 저장될 필요가 없다. 각 데이터(노드)는 메모리 어딘가에 흩어져 있어도 되고,
    대신 “포인터(pointer)” 로 서로 연결되어 있기 때문에 순서를 유지할 수 있다.
...
- 예시
  ...
  - 이런 경우 함수 형태로 인수에 넘겨주는 것이 훨씬 경제적. 초깃값이 없다면 함수를 실행해 무거운 연산을 시도, 초깃값이 존재한다면 함수를 실행하지 않고 기존 값 사용
...
- useEffect는 애플리케이션 내 컴포넌트의 여러 값들을 활용해 동기적으로 **부수 효과**를 만드는 메커니즘이다.
  즉, useEffect는 컴포넌트가 **렌더링 된 후에** 어떠한 부수 효과를 일으키고 싶을 때 사용하는 훅이다.
...
- 두개의 인수(콜백: 실행할 부수 효과가 포함된 함수, 의존성 배열)
  ...
  - 의존성 배열에 빈 배열을 넣으면 리액트가 이 useEffect는 비교할 의존성이 없다고 판단해 최초 렌더링(최초 마운트) 직후에 실행된 다음 부터는 더 이상 실행되지 않는다.
  - 아무런 값도 넘겨주지 않는다면, 렌더링 할 때마다 실행된다. 보통 컴포넌트가 렌더링 됐는지 확인하기 위한 방법으로 사용.
    ...
    - 의존성 배열이 없는 useEffect가 매 렌더링마다 실행된다면 그냥 useEffect 없이 써도 되는거 아닌가?
      ...
      - 반면 함수 내부에서의 직접 실행은 컴포넌트가 렌더링되는 도중에 실행된다. 따라서 useEffect를 사용했을 때와 달리, SSR의 경우 서버에서도 실행된다. 그리고 이 작업은 함수 컴포넌트의 반환을 지연시키는 행위다. 즉, 화면(UI)을 그리기 전에 실행되어 렌더링이 끝나기까지의 시간을 늘린다.
        따라서 무거운 작업일 경우 렌더링을 방해하므로 성능에 악영향.
...
- 클린업 함수는 생명주기 메서드의 언마운트 개념과는 조금 차이가 있다.
  - 언마운트는 컴포넌트가 DOM에서 사라진다는 것을 의미하지만 클린업 함수는 언마운트라기 보다는 함수 컴포넌트가 리렌더링됐을 때 이전 상태를 청소해 주는 개념
...
- eslint-disable-line react-hooks/exhaustive-deps 주석은 최대한 자제하라
  ...
  - useEffect는 반드시 의존성 배열로 전달한 값의 변경에 의해 실행돼야하는 훅이다.
    ...
    - 만약 특정 값을 사용하지만 해당 값의 변경 시점을 피할 목적이라면 메모이제이션을 적절히 활용해 해당 값의 변화를 막거나 적당한 실행 위치를 다시 한번 고민해 보는 것이 좋다. (메모이제이션(useMemo, useCallback)을 하면 참조가 유지되므로 의존성 배열에서 값이 변했다고 판단되지 않아 useEffect가 불필요하게 실행되지 않는다.)
...
- 거대한 useEffect를 만들지 마라
  ...
  - useEffect는 렌더링 시 의존성이 변경될 때마다 부수 효과를 실행한다. 이 부수 효과의 크기가 커질수록 애플리케이션 성능에 악영향.
    - useEffect가 컴포넌트의 렌더링 이후에 실행되므로 렌더링 작업에는 영향을 적게 미치지만 여전히 자바스크립트 실행 성능에는 영향을 미친다.
...
- useMemo와 useCallback의 유일한 차이는 메모이제이션을 하는 대상이 변수냐 함수냐 차이.
...
- useRef의 최초 기본값은 return문에 정의해둔 DOM이 아니고 useRef()로 넘겨받은 인수다.
  때문에 아래 예시에서 useRef가 선언된 당시에는 아직 컴포넌트가 렌더링되기 전이라 return으로 컴포넌트의 DOM이 반환되기 전이므로 undefined다.
...
- React Context를 안전하게 사용하는 패턴
  ...
  - useContext로 값을 불러올 때 Provider가 없는 상태(undefined) 를 미리 감지해서 예상치 못한 오류를 막는다.
    - useContext로 원하는 값을 얻으려고 했지만 정작 컴포넌트가 실행될 때 이 콘텍스트가 존재하지 않아 예상치 못하게 발생하는 에러를 방지하려면
      useContext 내부에서 해당 콘텍스트가 존재하는 환경인지, 즉, 콘텍스트가 한번이라도 초기화 되어 값을 내려주고 있는지 확인해보면 된다.
...
- use는 Promise나 Context와 같은 데이터를 참조하는 React API
- React Hook과 같이 use는 컴포넌트 또는 Hook에서만 호출해야 한다.
- 다만, React Hook과 달리 use는 if와 같은 조건문과 반복문 내부에서 호출할 수 있다.
- 매개변수: 참조하려는 데이터. 데이터는 Promise나 Context일 수 있다.
- 반환값: use Hook은 Promise나 Context에서 참조한 값을 반환한다.
...
- useState vs useReducer
  ...
  - useReducer = “상태 변화의 규칙을 미리 정의해두고, 그 규칙에 따라 상태를 바꿈”, useReducer는 의미 중심 (왜 / 어떤 액션으로 바꿀까?)
    ...
    |------|-----------|------------|
    | **목적** | 단순한 상태 값 관리 | 복잡한 상태 전이(transition) 로직 관리 |
    | **상태 구조** | 보통 한두 개의 독립된 값 | 여러 속성이 얽힌 복합 객체 |
    | **업데이트 방식** | `setState(newValue)` 직접 호출 | `dispatch(action)` → reducer 내부에서 상태 변경 정의 |
    | **로직 위치** | 컴포넌트 내부에 있음 | 컴포넌트 외부의 `reducer` 함수로 분리 |
    | **적합한 상황** | 폼 입력, 토글, 카운트 | 복잡한 폼 상태, 여러 이벤트에 따른 로직 분기, 상태머신 형태 |
...
- 단순히 DOM 엘리먼트 참조를 넘겨주는 게 아니라, 자식이 ref에 ‘행동’을 정의해서 부모에게 노출하는 방식
- 원래 ref는 {current: <HTMLElement>}와 같은 형태로 HTMLElement만 주입할 수 있는 객체지만 useImperativeHandle 훅을 사용해 전달받은 ref에 추가적인 동작을 정의할 수 있다.
...
- useImperativeHandle은 반드시 forwardRef와 함께 써야 한다.
  ...
  - useImperativeHandle은 이 ref를 “커스터마이징”하기 위한 훅. forwardRef를 사용해 ref를 받을 수 있게 되면
    useImperativeHandle을 사용해서 “ref.current에 들어갈 내용”을 직접 정의할 수 있다.
...
- React 19부터는 더 이상 forwardRef이 필요하지 않다. 이제 ref를 Prop으로 직접 전달하면 된다.
...
- useLayoutEffect는 모든 DOM의 변경 후에, 즉 리액트 렌더링 후에 useLayoutEffect의 콜백 함수 실행이 동기적으로 발생한다.

1. 리액트가 DOM을 업데이트 - 리액트의 렌더링(render phase)
2. useLayoutEffect를 실행
3. 브라우저에 변경 사항을 반영 - 브라우저의 렌더링(paint, 즉 화면 반영)
4. useEffect를 실행
...
- console.log()나 console.error()도 디버깅에 쓸 수는 있지만 useDebugValue는 “React 개발자 도구(React DevTools)” 전용의 디버깅 훅. 즉, 콘솔용이 아니라 리액트 훅의 내부 상태를 시각적으로 표시하기 위한 도구
  ...
  - 프로덕션에 콘솔을 제거하도록 설정할 수 있지만 제거 작업은 별도 플러그인 필요. 반면 useDebugValue 훅을 사용하면 기본적으로 프로덕션에서 아무 동작 안 함.
  - 콘솔로그는 호출 자체 비용이 발생(특히 반복되는 렌더 내에서). useDebugValue는 DevTools가 열릴때만 평가되므로 런타임 비용이 없다.

| 구분          | console.log / console.error      | useDebugValue                        |
| ------------- | -------------------------------- | ------------------------------------ |
| **출력 위치** | 브라우저 콘솔                    | React DevTools의 “Hooks” 탭          |
| **목적**      | 개발자에게 로그 남기기           | 커스텀 훅의 상태를 시각적으로 디버깅 |
| **사용 위치** | 아무 데서나 가능                 | 오직 커스텀 훅 내부                  |
| **실행 시점** | 코드가 실행될 때마다             | DevTools에서 해당 훅이 검사될 때     |
| **영향**      | 프로덕션 빌드에서도 남을 수 있음 | 개발 모드 전용 (빌드 시 영향 없음)   |
...
- 리액트 고차 컴포넌트는 with로 시작하는 이름을 사용해야 한다.
...
- 둘 다 불필요한 리렌더링을 줄이기 위한 도구이지만, “무엇을 메모이제이션(기억)하느냐”가 다르다.
  | 훅/함수 | 메모이제이션 대상 | 사용 위치 |
  |----------|------------------|------------|
  | **useMemo** | 값 (연산 결과) | 컴포넌트 내부 |
  | **React.memo** | 컴포넌트 전체 | 컴포넌트 외부 (컴포넌트 정의 시) |
...
- NGINX
  ...
  - 클라이언트의 웹 요청을 효율적으로 처리하고(요청을 받아 서버에 전달하고, 응답을 다시 돌려주는),
    트래픽을 제어하는 초고성능 웹 서버이자 리버스 프록시(Reverse Proxy)서버
  ...
  - NGINX 레벨에서 인증(Authentication) 을 걸면
    → 클라이언트가 아예 백엔드(Node.js, Next.js 등)에 도달하기 전에 차단할 수 있다.
    → 즉, 불필요한 자바스크립트 실행이나 API 호출 낭비가 줄어든다.
