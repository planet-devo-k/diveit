...
- JavaScript 엔진이 코드를 실행하기 전(런타임 전) 변수와 함수의 선언을 해당 스코프의 최상단으로 끌어올리는 것처럼 동작하는 현상.
...
- 모두 변수 선언에 사용되는 키워드
- 스코프
  - var는 함수 스코프만 인식하기 때문에 블록({ }) 안에서 선언해도 바깥에서 접근가능
  - let과 const는 블록 스코프를 가지기 때문에 해당 블록을 벗어나면 접근불가
- 중복 선언
  - var는 같은 스코프에서 여러 번 선언해도 에러가 나지 않고 마지막 값만 유효.
  - let과 const는 같은 스코프에서 중복 선언이 불가능
- 호이스팅
  - 세 가지 모두 변수 호이스팅은 되지만
  - var는 선언 전에도 호출 할 수 있고 값은 undefined로 나온다.
  - let과 const는 일시적 사각지대(TDZ)가 생겨 선언 전에 호출하면 에러.
...
- 함수가 실행될 때 ‘누가 이 함수를 호출했는지’에 따라 동적으로 결정되는 자기 참조 변수
...
- 자바스크립트는 동적 타입 언어라 유연한 반면, 실행 중에만 타입 에러를 잡아낼 수 있습니다.
- 반면 타입스크립트는 정적 타입 검사를 지원하기 때문에 실행 전에 오류를 잡을 수 있고 협업 시 코드의 의도를 명확히 전달할 수 있습니다.

#### TypeScript의 동작 원리에 대해 설명해 주세요.

- 타입스크립트 컴파일 과정

1. **타입스크립트 코드를 AST로 변환**하고 **타입 검사**를 통해 오류가 없는지 확인
2. 타입 검사가 통과되면 **자바스크립트 코드로 변환**

#### type을 선언하는 키워드들에는 무엇이 있고 차이가 무엇인지.

- TypeScript에서는 type, interface, class, enum 같은 키워드로 타입을 직접 선언할 수 있고
- 그 외에도 typeof, keyof, infer, extends 같은 타입 연산 키워드를 통해 타입을 조합하거나 변형하는 등 새로운 타입을 만들어낼 수도 있습니다.
