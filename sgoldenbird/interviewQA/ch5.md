...
1. MVC 패턴 (Model-View-Controller)
   데이터를 다루는 Model, 사용자에게 보여지는 View, 이 사이의 로직을 처리하는 Controller
...
2. Flux 패턴
   페이스북이 MVC의 예측 불가능한 데이터 흐름을 해결하기 위해 고안한 단방향 데이터 흐름(Unidirectional Data Flow) 패턴입니다.
...
<!--
낙관적 업데이트(Optimistic Update): 서버에 데이터를 요청한 뒤 응답이 올 때까지 기다리는 대신, 서버 응답이 성공할 것이라고 낙관적으로 가정하고 UI를 먼저 업데이트하는 기법.

| 단계 | 일반적인 업데이트 (Pessimistic) | 낙관적 업데이트 (Optimistic) |
| :--- | :--- | :--- |
| **1단계** | 사용자가 '좋아요' 버튼 클릭 | 사용자가 '좋아요' 버튼 클릭 |
| **2단계** | 서버에 요청을 보냄 (로딩 스피너 작동) | **즉시 UI의 좋아요 숫자를 +1로 변경** |
| **3단계** | 서버 응답이 올 때까지 대기 | 그동안 서버로 요청을 보냄 |
| **4단계** | 응답 성공 시 UI 업데이트 완료 | 서버 응답 성공 시 그대로 유지 (조용히 확정) |
| **5단계** | 응답 실패 시 에러 메시지 노출 | **서버 응답 실패 시 UI를 이전 상태로 롤백** |
-->
