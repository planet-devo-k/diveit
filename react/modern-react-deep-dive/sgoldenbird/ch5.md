...
- MVC 패턴에서는 데이터가 양방향으로 흘러 상태를 추적하고 이해하는 것이 매우 어려움
  - 뷰(HTML)와 모델(자바스크립트)이 서로를 변경할 수 있다보니 코드를 작성할때는 간단해도 관리하기는 어려움 → 단방향으로 데이터 흐름 변경
...
- action: 무엇을 할지 정의하는 객체
  - 어떤 작업을 처리할 액션과 그 액션 발생 시 함께 포함시킬 데이터. \
...
- **Flux구조** + Elm 아키텍처
  - Elm: 웹페이지를 선언적으로 작성하기 위한 언어. \
    Flux와 마찬가지로 데이터 흐름을 단방향으로 강제하고 세가지로 분류.
    - **Model**: 애플리케이션의 상태
    - **View**: 모델을 표현하는 HTML
    - **Update**: 모델을 수정하는 방식
- 리덕스는 <mark style="background-color:yellow;">**하나의 글로벌 상태 객체를 스토어에 저장**</mark>해두고 <mark style="background-color:yellow;">**이 객체를 업데이트**</mark>하는 작업을 디스패치해 업데이트 수행
...
- ContextAPI는 **상태 관리가 아니라** **전역&#x20;**<mark style="background-color:yellow;">**상태 주입**</mark>을 도와주는 역할
...
- **왜 사용하는가? (핵심 가치)**
  ...
  가장 큰 이유는 서버 데이터와 클라이언트 UI 사이의 <mark style="background-color:yellow;">**동기화**</mark>입니다.
  - 캐싱(Caching): 동일한 데이터를 여러 번 요청할 때 서버에 가지 않고 메모리에서 즉시 꺼내 씁니다.
  - 데이터 신선도 관리: 데이터가 상했는지(Stale), 신선한지(Fresh) 판단하여 자동으로 백그라운드에서 업데이트합니다.
  - 로딩/에러 상태 관리: `isLoading`, `isError` 같은 상태를 직접 만들 필요 없이 라이브러리가 제공해 줍니다.
  - 불필요한 요청 방지: 포커스가 다시 들어왔을때나 네트워크가 재연결되었을때만 똑똑하게 데이터를 다시 가져옵니다.

**Stale-While-Revalidate (SWR) 전략**이란?

라이브러리의 이름이기도 한 이 전략은 사용자 경험을 극대화하는 방식입니다.

1. Stale: 먼저 캐시에 있는 낡은 데이터를 즉시 화면에 보여줍니다. (**사용자는 기다리지 않음**)
2. While: 그동안 백그라운드에서 조용히 서버에 최신 데이터를 요청합니다.
3. Revalidate: 서버에서 새로운 데이터가 오면 최신값으로 화면을 교체합니다.

이 과정을 통해 사용자는 하얀 화면(로딩 중)을 거의 보지 않고 **부드러운 화면 전환**을 경험하게 됩니다.

**Redux**: 컴포넌트 간에 공유되는 UI 상태나 사용자 입력값 관리 (**Client State**)
\
**React Query/SWR**: <mark style="background-color:yellow;">**DB에서 가져온 서버 데이터**</mark>의 캐싱 및 동기화 관리 (**Server State**)

### Recoil, Zustand, Jotai, Valtio

- 기존의 리덕스 같은 라이브러리와 달리 훅을 활용해 작은 크기의 상태를 효율적으로 관리
- 기존 상태 관리 라이브러리의 아쉬운 점으로 지적받던 <mark style="background-color:yellow;">**전역 상태 관리 패러다임에서 벗어나**</mark> 개발자가 **원하는 만큼의 상태를&#x20;**<mark style="background-color:yellow;">**지역**</mark>**적으로 관리**하는 것을 가능하게 만들었다.

### <mark style="background-color:orange;">데이터의 성격에 따라 '역할 분담'</mark>

데이터는 크게 두 종류로 나뉩니다.

<table><thead><tr><th width="136.99993896484375">구분</th><th width="263.5999755859375">서버 상태 (Server State)</th><th>클라이언트 상태 (Client State)</th></tr></thead><tbody><tr><td><strong>데이터 예시</strong></td><td>유저 목록, 게시글 내용, 주문 내역</td><td>모달 열림/닫힘, 다크모드 설정, 입력 폼 텍스트</td></tr><tr><td><strong>관리 라이브러리</strong></td><td>React Query, SWR</td><td>Zustand, Redux, Jotai</td></tr><tr><td><strong>핵심 과제</strong></td><td>캐싱, 비동기 통신, 데이터 최신화</td><td>컴포넌트 간 데이터 공유, UI 동기화</td></tr></tbody></table>

#### 같이 썼을 때의 구체적인 장점

① Redux/Zustand가 훨씬 가벼워집니다. \
기본적으로 서버에서 데이터를 가져오면 로딩 중인지, 에러가 났는지 등의 상태를 직접 만들어야 했습니다.

- 기존: `isFetching`, `error`, `data` 등의 상태를 Redux에 일일이 정의함 (보일러플레이트 증가)
- 함께 사용: 서버 데이터는 React Query가 알아서 관리하고, Redux에는 정말 필요한 전역 UI 상태만 남깁니다. 코드가 획기적으로 줄어듭니다.

② 캐싱 전략의 차이 Zustand나 Redux는 단순히 데이터를 메모리에 저장할 뿐, 이 데이터가 "언제 상했는지(stale)"는 모릅니다. 반면 React Query나 SWR은 일정 시간이 지나면 자동으로 데이터를 다시 가져오는 등 서버와의 동기화에 최적화되어 있습니다.

#### 실제 협업 시나리오 (예시)

1. React Query: 게시판 목록 데이터를 서버에서 가져와 캐싱합니다. (서버 상태)
2. Zustand: 사용자가 게시글 필터링을 위해 선택한 '카테고리'나 '검색어'를 저장합니다. (클라이언트 상태)
3. 결합: React Query의 API 호출 함수에서 Zustand에 저장된 '검색어' 상태를 읽어와서 서버에 요청을 보냅니다.

---

<table><thead><tr><th width="123.4000244140625">항목</th><th>서버 상태 관리 (React Query/SWR)</th><th>전역 상태 관리 (Zustand/Redux)</th></tr></thead><tbody><tr><td><strong>주요 대상</strong></td><td>데이터베이스(DB)에서 가져온 외부 데이터</td><td>브라우저 메모리에만 존재하는 UI 데이터</td></tr><tr><td><strong>소유권</strong></td><td><strong>서버와 클라이언트가 공유</strong> (Shared)</td><td>클라이언트가 독점 (Local)</td></tr><tr><td><strong>특징</strong></td><td>비동기 요청, 캐싱, Revalidate 중심</td><td>동기적 업데이트, <br>컴포넌트 간 Prop Drilling 해결</td></tr><tr><td><strong>함께 쓰는 이유</strong></td><td>역할 분리를 통해 상태 관리 로직의 복잡성을 낮추고 유지보수성을 높이기 위함</td><td></td></tr></tbody></table>
...
### 지역 상태의 한계를 벗어나 useState의 상태를 바깥으로 분리하기

함수 외부에서 상태를 참조하고, 이를 통해 렌더링까지 자연스럽게 일어나려면 아래 조건들을 만족시켜야 한다. \
(리렌더링이 되야 업데이트된 상태를 컴포넌트에 반영시킬 수 있다.)
...
- Recoil, Jotai
  - <mark style="background-color:yellow;">**Context와 훅을 기반**</mark>으로 작은상태를 효율적으로 관리하는데 초점
- Zustand
  - 리덕스 비슷하게 **하나의 큰 스토어**를 기반으로 상태 관리
  - Recoil, Jotai와는 다르게 이 **하나의 큰 스토어는&#x20;**<mark style="background-color:yellow;">**Context가 아니라 스토어가 가지는 클로저를 기반**</mark>으로 생성
...
- Jotai는 **상향식(bottom-up)** 접근법을 취하고 있다. 이는 리덕스처럼 하나의 큰 상태를 애플리케이션에 내려주는 방식이 아니라 작은 단위의 상태를 위로 전파하는 구조.
- 리액트 **Context의 문제점인 불필요한 리렌더링을 해결**하고자 설계돼 있다.
  - 개발자들이 메모이제이션이나 최적화를 거치지 않아도 리렌더링이 발생되지 않도록 설계
- atom
  - 최소 단위의 상태
  - 리코일과는 다르게 atom하나만으로도 상태를 만들수도 있고 또 이에 파생된 상태를 만들 수도 있다.
  - Jotai는 각 atom을 생성할 때마다 고유한 key를 필요로 했던 recoil과는 달리 \
...
- useAtomValue
  ...
  - atom값은 store에 존재한다. store에 **atom객체 그 자체를 키로 활용**해 값을 저장한다. \
    이런 방식을 위해 WeakMap이라고 하는 자바스크립트에서 객체만을 키로 가질수 있는 독특한 방식의 Map을 활용해 recoil과는 다르게 별도의 key를 받지 않아도 스토어에 값을 저장할 수 있다.
...
- 리덕스에 영감을 받아 만들어짐
- **하나의 스토어를 중앙 집중형으로 활용**해 이 스토어 내부에서 상태를 관리
- store는 리액트를 비롯한 그 어떤 프레임워크와는 별개로 완전히 독립적으로 구성. \
  바닐라 자바스크립트 환경에서도 사용 가능
...
- subscribe는 listener를 등록. \
  상태값이 변경될 때 리렌더링이 필요한 컴포넌트에 전파. subscribe를 통해 스토어의 값이 변경될 때마다 특정 함수를 실행할 수도 있다. subscribe는 현재 값과 이전 값 둘 다 확인할 수 있으므로 특정 값이 변경될 때만 실행되게 최적화할 수도 있다.
...
- 특징
  ...
  - zustand 자체의 라이브러리 크기도 다른 라이브러리에 비해 굉장히 가볍다. API가 복잡하지 않고 사용이 간단해 쉽게 접근할 수 있다.
  - 마찬가지로 **타입스크립트 기반으로 작성**돼 있어서 별도로 @types를 설치하거나 임의로 작성된 d.ts에 대한 우려 없이 타입스크립트를 자연스럽게 쓸 수 있다.
  - 리덕스와 마찬가지로 미들웨어를 지원. create의 두번 째 인수로 원하는 미들웨어를 추가하면 된다.
    - 스토어 데이터를 영구히 보존할 수 있는 persist
    - 본잡한 객체를 관리하기 쉽게 도와주는 immer
    - 리덕스와 함께 사용할수 있는 리덕스 미들웨어 등

## 기타

- tearing: **하나의 상태에 따라 서로 다른 결과물을 사용자에게 보여주는 현상**. 일종의 레이스 컨디션(Race Condition, 경쟁 상태) 때문에 발생하는 현상
- MVC 패턴: 소프트웨어를 만들 때 그 역할을 Model(모델), View(뷰), Controller(컨트롤러) 세 가지로 나누어 설계
  - Model: 데이터와 비즈니스 로직
    - 앱이 무엇을 할지, 어떤 데이터를 가질지 정의합니다.
    - DB와 연동하여 데이터를 저장, 수정, 불러오는 일을 하며, 화면(UI)이 어떻게 생겼는지는 전혀 신경 쓰지 않습니다.
  - View: 화면 UI
    - 사용자가 실제로 보는 화면(텍스트 박스, 버튼 등)입니다.
    - 모델이 가진 데이터를 받아서 시각적으로 보여주기만 합니다. 데이터를 직접 수정하지 않고 보여주는 역할에 충실합니다.
  - Controller: 중재자
...
- 메인테이너(Maintainer): 오픈소스 소프트웨어 프로젝트를 책임지고 관리하는 사람(또는 그룹)
