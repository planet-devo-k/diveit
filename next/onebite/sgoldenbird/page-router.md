...
![리액트에서의 데이터 fetching](<../.gitbook/assets/react-datafetching (2).png>) ![리액트에서의 데이터 fetching2](<../.gitbook/assets/react-datafetching2 (2).png>)

### 넥스트 data fetching

* 넥스트는 다양한 pre rendering 방식 제공
  * 사전 렌더링 중 요청이 들어왔을 때 JS 렌더링 단계에서 data fetching도 미리 하도록 설정 가능 ![넥스트에서의 데이터 fetching](<../.gitbook/assets/next-datafetching (2).png>)
  * 만약 데이터 용량이 엄청 크거나 서버 이상 등으로 fetch 시간이 오래걸리면 유저는 그동안 빈화면을 보게됨 → 넥스트는 빌드타임에 사전렌더링을 하도록 설정할 수도 있다. SSG ![넥스트 빌드타임 사전렌더링, 데이터 fetching](<../.gitbook/assets/next-build-datafetching (2).png>) ![넥스트의 다양한 사전 렌더링 방식 SSR, SSG, ISR](<../.gitbook/assets/next-various-pre-rendering (2).png>)
...
*   매번 같은 페이지만 응답함. 최신 데이터 반영은 어렵다.
    ...
    ![ssg1](<../.gitbook/assets/ssg1 (2).png>) ![ssg2](<../.gitbook/assets/ssg2 (2).png>)
...
*   동적 경로를 갖는 페이지를 getStaticProps로 SSG로 만들고 싶다면 getStaticPaths 필요
    ...
    * getStaticPaths에서 경로를 아래처럼 반환
    ...
    ![동적 경로에 SSG 적용하기1](<../.gitbook/assets/dynamic-ssg1 (2).png>)
* fallback 옵션
...
![fallback blocking](<../.gitbook/assets/fallback-blocking (2).png>) ![fallback true](<../.gitbook/assets/fallback-true (2).png>)
...
* 요청을 받을때마다 페이지를 다시 생성하는 ISR인 on-Demand-ISR 사용 ![on-Demand-ISR](<../.gitbook/assets/on-demand-isr (2).png>)
...
* 불필요한 컴포넌트: 상호작용 기능이 없어서 브라우저에서 한 번 더 실행이 되어 하이드레이션 될 필요 없는 컴포넌트들
  *   넥스트 컴포넌트들 2번 실행
      ...
      * 브라우저 측에서 하이드레이션을 위해 한번 더 실행
      ...
      ![넥스트 컴포넌트 2번 실행](<../.gitbook/assets/twice (2).png>)
...
* 넥스트의 컴포넌트들은 상호작용이 있는 컴포넌트와 없는 컴포넌트들이 있는데 없는 컴포넌트들이 더 많다. 상호작용 없는 컴포넌트들은 단순히 UI만 렌더링. 상호작용이 없는 컴포넌트들은 사전 렌더링에서 HTML로 변환만 되면 됨. JS Bundle에 포함되어 브라우저에서 한번 더 실행될 필요가 없다.\
  → JS bundle 용량이 커져서 하이드레이션도 느려지고 결국 TTI 늦어짐
