...
- unknown은 모든 값을 할당할 수 있는 top type으로 어떠한 값도 할당 불가능
- typeof를 이용하여 타입 좁히기를 통해 사용됨

**타입 가드를 적극 활용하자**

- instanceof : 특정 클래스의 인스턴스 확인 연산자
- typeof : 특정 요소 자료형 확인
- in : 객체에 키가 존재하는지 확인

**제네릭**

- 함수나 클래스 내부에서 단일 타입이 아닌 다양한 타입에 대응 할 수 있게 도와주는 도구
- 일반적으로는 T,U로 표현하는 경우가 있는데 적절히 네이밍 하면 좋음

```ts
function get<T>(list: T[]): [T, T] {
...
- 객체 키의 타입을 부여할 수 있는 방식
- 동적인 객체를 정의할 때 유용하지만 키의 범위가 클 경우에는 필요에 따라서 타입을 좁혀야 한다

**방법**

1. Record<Key, Value> : 객체 타입의 각각 원하는 키와 값을 넣는 것이 가능하다.
2. Object.keys: 해당 함수는 문자열 배열로 반환해 주기 때문에 에러가 발생할 수 있다. 해결 방법은 아래와 같다  
   a. as로 타입 단언 하기  
   b. 타입 가드 함수 만들기  
   c. 가저온 키를 keysOf를 통하여 객체의 키를 가지고 오기

### 타입스크립트 전환 가이드

1. tsconfing.json 먼저 작성하기
